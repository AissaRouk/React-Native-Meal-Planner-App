...
\begin{itemize}
    ...
    \item \textbf{Referencias y documentación.} Las bases teóricas del \emph{meal\,planning} y de la usabilidad provienen de estudios recientes y obras de referencia citadas en la bibliografía, como los trabajos de la Organización Mundial de la Salud sobre obesidad y sobrepeso, las investigaciones de la Biblioteca Nacional de Medicina sobre planificación dietética【382000137647362†L372-L383】 y los principios de usabilidad de Nielsen【382000137647362†L410-L418】.  Además se consultaron la documentación oficial de React Native, Firebase y MiniSearch.
\end{itemize}
...
\chapter{Diseño y descripción del sistema}\label{cap.diseno y descripcion del sistema} %En este apartado se detallarían decisiones de diseño e implementación, como la decisión de utilizar React-Native y explicar por qué se ha elegido ese motor sobre otros
	\section{Lógica de la aplicación}
		...
		\subsection{Justificación de los tipos y colecciones empleadas}
		
			% ...existing code...
			\subsubsection{Motivación general de los tipos definidos en \texttt{src/Types/Types.tsx}}
			...
			\subsection{Diseño y lógica de las páginas}
			...
			\subsection{Componentes de la aplicación}
				\subsubsection{Componentes de la interfaz}
					\paragraph{FloatingButton\\}
						Componente \texttt{FloatingButton}
						\label{FloatingButton}
						
						\textit{Floating Button} es un botón flotante — componente redondo con sombra— diseñado para acciones primarias en pantalla. El componente ofrece una apariencia de flotación mediante elevación y sombra, y está implementado en \texttt{FloatingButton.tsx} como una función que devuelve el elemento táctil con un icono centrado.
						...
						\begin{figure}[H]
							\centering
							% Ajusta la ruta si guardas la imagen en latex/figures/
					...
					\textbf{Descripción funcional}
					\begin{itemize}
						\item \texttt{IngredientCard} muestra el nombre del ingrediente y la cantidad requerida por la receta; su diseño prioriza la legibilidad y la consistencia visual con el resto de la interfaz.
						\item No permite la edición directa de los campos que muestra; cualquier modificación debe realizarse a través de los flujos de edición correspondientes de la aplicación.
						\item Se utiliza en la página \hyperref[RecipeScreen]{RecipeScreen} y en el modal \hyperref[AddRecipeModal]{AddRecipeModal} como elemento de la lista de ingredientes de una receta.
					\end{itemize}
					
					\textbf{Interfaz pública (props) y contrato}
					\begin{itemize}
						\item \textbf{Prop principal:} un objeto de tipo \texttt{RecipeIngredient} (o similar) que contiene al menos \texttt{id}, \texttt{name}, \texttt{quantity} y \texttt{quantityType}.
						\item \textbf{Contrato:} entrada = datos de un \texttt{RecipeIngredient}; salida = renderizado visual; efectos secundarios = ninguno. El componente garantiza no mutar los datos recibidos y no invocar callbacks de modificación.
						\item \textbf{Precondiciones:} el objeto recibido debe contener un identificador único y un nombre legible; la cantidad puede ser numérica o nula (el componente debe mostrar \texttt{N/A} o un marcador equivalente cuando falten datos).
					\end{itemize}
					
					\textbf{Estructura y dependencias}
					\begin{itemize}
						\item Es una función / componente funcional escrito en TypeScript/React Native.
						\item Depende únicamente de primitives de React Native para el renderizado (por ejemplo, \texttt{View}, \texttt{Text}, \texttt{StyleSheet}) y de los tipos definidos en \texttt{Types.tsx} para tipado robusto.
						\item No realiza llamadas a servicios ni accede al contexto de la aplicación; su lógica se limita a la presentación.
					\end{itemize}
					...
					\textbf{Consideraciones de robustez y casos borde}
					\begin{itemize}
						\item \textbf{Datos incompletos:} mostrar marcadores claros cuando la información sea parcial.
						\item \textbf{Longitud del texto:} truncar nombres excesivamente largos o usar \texttt{numberOfLines} para evitar desbordes en la UI del modal o la pantalla.
					\end{itemize}
					...
					\begin{figure}[H]
						...
						\caption{Ejemplos de varios componentes \textit{IngredientCard}}
					...
					El componente \texttt{AppHeader} se utiliza para mostrar las cabeceras en las distintas pantallas de la aplicación. Está diseñado como un componente funcional que recibe un texto para mostrar como título y, opcionalmente, un componente que se situará a la derecha del título (por ejemplo, un botón de acción o un icono). Su objetivo es proporcionar una presentación consistente de las cabeceras y facilitar la reutilización en todas las vistas.
					
					\textbf{Descripción funcional}
					\begin{itemize}
						\item Muestra un título claramente legible, con estilo destacado, alineado a la izquierda.
						\item Permite la inserción de un componente auxiliar en la zona derecha de la cabecera (por ejemplo, un botón de cerrar sesión, acceso a un menú o un icono de ayuda).
						\item Mantiene un diseño sencillo y adaptable para diferentes tamaños de pantalla.
					\end{itemize}
					...
					\textbf{Estructura y uso}
					\begin{itemize}
						\item Implementado con \texttt{View} y \texttt{Text} de React Native, y estilizado mediante \texttt{StyleSheet}.
						\item Está pensado para ser incluido en la parte superior de las pantallas principales de la aplicación, proporcionando uniformidad visual.
						\item La inclusión de componentes auxiliares a la derecha permite adaptarlo a distintos flujos funcionales sin alterar el diseño base.
					\end{itemize}
					...
					El componente \texttt{HeaderComponent} se utiliza como cabecera específica de la pantalla principal (\hyperref[MainScreen]{MainScreen}). Está compuesto por una cabecera superior reutilizable (\texttt{\hyperref[AppHeader]{AppHeader}}) y una vista deslizable horizontal que presenta los días de la semana para permitir al usuario seleccionar el día deseado. Su propósito es ofrecer una navegación rápida entre los días y mantener coherencia visual con el resto de la interfaz.
					
					\textbf{Descripción funcional}
					\begin{itemize}
						\item Muestra una fila superior con el título de la sección y un botón de acción a la derecha (gestionado por \texttt{AppHeader}).
						\item Debajo de la cabecera muestra un \texttt{ScrollView} horizontal con tarjetas representando cada día de la semana; el día seleccionado se resalta visualmente (color de acento) para indicar la elección del usuario.
						\item Está pensado exclusivamente para \hyperref[MainScreen]{MainScreen} y centraliza la lógica de selección del día en la UI de inicio.
					\end{itemize}
					
					\textbf{Parámetros y contrato}
					El componente se define como una función que recibe los siguientes parámetros:
					\begin{itemize}
						\item \texttt{selectedDay: DaysOfWeek} — día actualmente seleccionado por el usuario; se usa para aplicar el estilo activo a la tarjeta correspondiente.
						\item \texttt{setSelectedDay: (day: DaysOfWeek) => void} — función que actualiza el día seleccionado en el estado del contenedor.
						\item \texttt{onButtonPress: () => void} — callback que se ejecuta al pulsar el botón situado en la esquina superior derecha de la cabecera (comportamiento delegado al contenedor).
					\end{itemize}
					...
					\textbf{Estructura y dependencias}
					\begin{itemize}
						\item Internamente combina \texttt{AppHeader} para la zona superior y un \texttt{ScrollView} horizontal para las tarjetas de los días.
						\item Utiliza el arreglo de días exportado (\texttt{daysOfWeekArray}) para renderizar las tarjetas de selección.
						\item Para la presentación emplea componentes nativos (\texttt{View}, \texttt{Text}, \texttt{TouchableOpacity}) y estilos mediante \texttt{StyleSheet}. La iconografía se obtiene de la librería de iconos que usa la aplicación.
					\end{itemize}
				...
				\label{subsec:IngredientComponent}
				
				A continuación se explica, de manera concisa y práctica, el funcionamiento del componente \texttt{IngredientComponent}. Este componente es una unidad de interfaz que recibe como entrada un objeto \texttt{RecipeIngredient} (identificado por su \texttt{id}) junto con callbacks para actualizar cantidad, tipo de unidad y para eliminar el ingrediente. Su responsabilidad es presentar el nombre, la cantidad y la métrica del ingrediente y ofrecer controles de interacción (botones y campo numérico) que delegan la lógica de mutación en el componente contenedor.
				
				\paragraph{Descripción funcional}
				\begin{itemize}
					\item Muestra el nombre legible del ingrediente (resuelto a partir de la lista completa \texttt{ingredients} pasada como prop).
					\item Presenta un control de cantidad editable: botones de incremento/decremento y un campo de texto numérico con teclado decimal.
					\item Muestra un selector (\hyperref[CustomPicker]{Picker}) para elegir el tipo de unidad asociado a la cantidad.
					\item Incluye un botón para eliminar el ingrediente; todas las acciones disparan callbacks proporcionados por el contenedor, por lo que la persistencia del estado se realiza fuera del componente.
				\end{itemize}
				
				\paragraph{Interfaz (props) y contrato}
				El componente acepta las siguientes props (tipado en TypeScript, tal y como aparece en el código):
				\begin{itemize}
					\item \texttt{ingredients: Ingredient[]} — lista completa de ingredientes disponibles (usada para resolver el nombre por \texttt{id}).
				...
				\paragraph{Estructura interna y dependencias}
				\begin{itemize}
					\item Implementado como componente funcional en TypeScript/React Native.
					\item Usa primitivas de React Native: \texttt{View}, \texttt{Text}, \texttt{TextInput}, \texttt{TouchableOpacity} y estilos mediante \texttt{StyleSheet}.
					\item Utiliza \texttt{Icon} (Ionicons) para los botones gráficos y un componente local \texttt{CustomPicker} para seleccionar el \texttt{quantityType}.
					\item Depende de los tipos definidos en \texttt{Types.tsx} (por ejemplo, \texttt{Ingredient} y \texttt{QuantityType}).
				\end{itemize}
				
				\paragraph{Comportamiento y detalles de UX}
				\begin{itemize}
					\item El componente mantiene internamente \texttt{textValue} (string) para el campo de texto de la cantidad y sincroniza con \texttt{quantity} mediante \texttt{setQuantity} cuando el valor es válido.
					\item Al editar el campo numérico se valida con una expresión regular que permite dígitos y punto decimal; en \texttt{onBlur} se restablece el texto al valor numérico actual si la entrada no es válida.
					\item Los botones \texttt{remove} y \texttt{add} calculan el nuevo valor (asegurando un mínimo de 0), actualizan la cantidad mediante \texttt{setQuantity} y sincronizan el \texttt{textValue}.
					\item El \texttt{CustomPicker} controla el tipo de unidad a través de \texttt{pickerOpen} y \texttt{setQuantityType}; la lista de opciones usada es \texttt{quantityTypes}.
					\item Se aplica \texttt{numberOfLines} y \texttt{ellipsizeMode="tail"} al nombre para evitar desbordes y preservar la alineación en listas o modales.
				\end{itemize}
				
				\paragraph{Consideraciones de robustez y accesibilidad}
				\begin{itemize}
					\item Validar que \texttt{ingredients.find(...)} devuelva un nombre antes de renderizar; mostrar un marcador (p. ej. \texttt{N/A}) si no se encuentra.
					\item Usar \texttt{recipeIngredient.id} como clave en los mapeos de lista; evitar índices como \texttt{key}.
					\item Añadir \texttt{accessible} y \texttt{accessibilityLabel} en elementos interactivos (botones) para mejorar compatibilidad con lectores de pantalla.
					\item Para colecciones grandes, considerar memoización del componente y usar \texttt{FlatList} con \texttt{getItemLayout} para optimizar el renderizado.
				\end{itemize}
				...
				\subsubsection{Componentes modales}
					...
					El pequeño componente \texttt{AddIngredientButton} actúa como disparador de la creación rápida de ingredientes desde contextos donde el usuario está añadiendo recetas o editando la despensa. Su diseño es compacto y reconocible (icono "+") y su responsabilidad es abrir el modal de creación de ingrediente, centralizando la lógica de navegación y manteniendo la UI limpia. Su existencia evita repetir el mismo control en múltiples lugares y mejora la coherencia de la interfaz.
					...
					\paragraph{ModalHeareComponent}
					El componente \texttt{ModalHeareComponent} (sic. nombre según el repositorio) actúa como cabecera reutilizable dentro de los modales complejos. Provee el título, el botón de cierre y, opcionalmente, atajos contextuales (por ejemplo, guardar o retroceder). Su uso mejora la coherencia entre los distintos modales y reduce la repetición de código para la gestión de la zona superior de estas ventanas.
					...
					\paragraph{PlannedIngredeintOptionsModal}
...
% En este capítulo se describen las iteraciones de desarrollo de NutriPlan.  Cada prototipo representa
% un hito en la evolución de la aplicación, incorporando nuevas funcionalidades y refinando la experiencia de usuario.
  \section{Prototipo 1}
  El primer prototipo se centró en construir la base de la aplicación.  En esta etapa se implementó la autenticación de usuarios mediante Firebase, permitiendo el registro e inicio de sesión.  Se diseñaron las pantallas \textit{LoginScreen} y \textit{RegisterScreen} con un estilo minimalista y se desarrolló la página principal (\textit{MainScreen}) como punto central de navegación.  También se creó la funcionalidad básica de gestión de recetas: el usuario podía añadir una nueva receta con su título, enlace y porciones, visualizar la lista de recetas en \textit{RecipesScreen} y acceder al detalle en \textit{RecipeScreen}.  Se definieron las colecciones en Firestore y se validaron las operaciones CRUD básicas.  Este prototipo sirvió para sentar las bases de la arquitectura en capas y de la navegación entre pantallas.

  \section{Prototipo 2}
  En el segundo prototipo se ampliaron notablemente las funcionalidades.  Se añadió la planificación semanal, permitiendo asignar recetas o ingredientes a días y comidas mediante la entidad \texttt{WeeklyMeal}.  Para ello se crearon componentes de selección de día y tipo de comida en la \textit{MainScreen} y se desarrolló el \textit{PlanMealModal}.  Se incorporó la gestión de despensa (\textit{PantryScreen}), donde el usuario puede visualizar y modificar los ingredientes disponibles, así como la generación automática de la lista de la compra (\textit{GroceryListScreen}).  Se implementó el algoritmo que suma las cantidades de ingredientes planificados y resta las existencias de la despensa, consolidando resultados en una lista ordenada【382000137647362†L1773-L1794】.  Además se integró \texttt{react-minisearch} para la búsqueda local de ingredientes, mejorando la experiencia de añadir elementos a las recetas y a la despensa.  Con este prototipo se validó la viabilidad de los casos de uso principales.
