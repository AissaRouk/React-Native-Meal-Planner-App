...
\chapter*{}
\begin{flushright}
\textit{Dedicado a mi madre, Mariem El Masoudi, y a mi pareja, Eirini Michaleakou, por haberme ayudado y apoyado en todo.}
\end{flushright}
\thispagestyle{empty}
...
\thispagestyle{empty}
La Organización Mundial de la Salud (OMS) cita que en 2022 el 43\% de los adultos en el mundo sufría de sobrepeso, y el 16\% padecía obesidad, lo que equivale a unos 890 millones de adultos obesos\cite{OMSObesidadySobrepeso}. Desde 1990, el número de personas con obesidad se ha más que duplicado entre los adultos y se ha cuadruplicado entre los adolescentes\cite{ONUAAComercioAlimentosyObesidad}.
\\Esta situación afecta a personas de todas las edades, regiones y tiene graves consecuencias para la salud, la economía y los sistemas sanitarios; y hace cada vez más urgente fomentar hábitos alimentarios saludables y estrategias que promuevan una alimentación consciente, eficiente y sostenible.
\\De este contexto surge NutriPlan, una aplicación de \textit{Meal Planning} y gestión de recetas cuyo objetivo es fomentar un estilo de vida saludable, económico y sostenible, reduciendo el desperdicio de alimentos. 
Esta idea surgió en mi experiencia como estudiante Erasmus, donde descubrí que al realizar la lista de la compra manualmente y verificar cuáles ingredientes tenía, se ahorraba más dinero en la compra, desperdiciaba menos comida y lo más importante, comía sano.
\\
\textbf{Palabras clave:} planificación dietética, gestión de recetas, aplicaciones móviles, React Native, sostenibilidad, usabilidad, desperdicio de alimentos.
...
\thispagestyle{empty}
The World Health Organization (WHO) reports that in 2022 43\% of adults in the world are overweight, and 16\% of them suffered from obesity, which is equivalent to aproximately 890 million obese adults\cite{OMSObesidadySobrepeso}. Since 1990 the number of people with obesity have more than doubled in adults and quadrupled among adolescents\cite{ONUAAComercioAlimentosyObesidad}.
\\This situation affects people from all ages, regions and has serious consequences on health, the economy, and healthcare systems; making it increasingly urgent to promote healthy eating habits and strategies to promote a conscious, efficient and sustainable feeding.
\\NutriPlan stems from this context, a \textit{meal planning} and recipe managing app whose goal is to encourage a healthy, sustainable and economical lifestile.\\
This idea came from my Erasmus exchange, where I discovered that when I made a grocery list and verified the ingredients I had in my pantry before doing the groceries themselves, I saved more money in these, I wasted less food and most importantly, I ate healthy.

\textbf{\\Key words:} meal planning, recipe management, mobile apps, React Native, sustainability, usability, food waste.
...
\chapter{Introducción}\label{cap.introduccion}
  ...
  \section{Estructura de la memoria}
  	...
  	El capítulo de \emph{Prototipos y desarrollo} explica la evolución del proyecto.  Para cada versión se describe qué funcionalidades se implementaron, cómo evolucionó la interfaz y qué problemas técnicos se resolvieron.
  ...
  \end{itemize}
	...
	\section{Planificación temporal}
		...
		\begin{enumerate}
			...
			\item \textbf{Implementación del prototipo 1.2 (semanas 9–12).} Se añadieron la funcionalidad de planificación semanal, la despensa y la generación de la lista de la compra.  Se integró MiniSearch para la búsqueda de ingredientes y se ajustó la estructura de datos para permitir la consolidación de cantidades.
			\item \textbf{Implementación del prototipo 1.3 (semanas 13–15).} Se completaron las funcionalidades pendientes: subida de imágenes para las recetas, edición de ingredientes desde la despensa, marcaje de elementos comprados y mejoras de usabilidad.  También se realizaron pruebas de integración y se refinaron los estilos y la navegación.
...
\chapter{Estado del arte}\label{cap.estado del arte}
	...
	\section{Diseño de la interfaz}
		...
		\begin{enumerate}
			...
			\item \textbf{Funcionalidad:} enfatiza que cada componente en el diseño debe tener un propósito claro haciendo que se eliminen elementos decorativos que no aportan ningún propósito claro haciendo que se eliminen elementos decorativos que no aportan ningún valor funcional.
	...
	%Nombrar aplicaciones que hayan resultado de inspiración para el proyecto y explicar un poco de qué van esas aplicaciones
	  \subsection*{Aplicaciones móviles existentes de \textit{meal planning}}
...
\chapter{Diseño y descripción del sistema}\label{cap.diseno y descripcion del sistema} %En este apartado se detallarían decisiones de diseño e implementación, como la decisión de utilizar React-Native y explicar por qué se ha elegido ese motor sobre otros
	...
	\section{Diseño de las capas aplicación}
		...
		\paragraph{\\Integración con Firebase\\}
		...
		\begin{itemize}
			...
			\item Simplifica el testing. 
		\end{itemize}
		
		\paragraph{\\Gestión de estado global\\}
		...
		\paragraph{\\Inicialización y ciclo de vida\\}
		...
		\paragraph{Detalles de implementación y funciones principales\\}
		La definición del contexto reside en el archivo \texttt{Context.tsx}, donde se declara el tipo \texttt{ContextProps} con todas las propiedades de estado y los métodos de acceso. Entre las propiedades se incluyen el identificador del usuario (\texttt{userId}), las listas de ingredientes y recetas, la despensa, los elementos marcados como comprados y la planificación semanal. El contexto también expone métodos asíncronos como \texttt{addOrUpdateIngredient}, que decide si un ingrediente se crea o se actualiza en Firestore y sincroniza el estado local; \texttt{getAllRecipes}, que recupera todas las recetas del usuario ordenadas por fecha; y \texttt{addWeeklyMeal}, que asigna un plato o ingrediente a un día concreto de la semana y actualiza automáticamente la lista de ingredientes planificados. Otras funciones gestionan operaciones relacionadas, como eliminar ingredientes de una receta, crear múltiples relaciones en lote o sincronizar las compras con el inventario.
		
		\paragraph{Arquitectura y motivos de diseño\\}
		La implementación de \texttt{AppProvider} utiliza hooks de React (\texttt{useState}, \texttt{useEffect}) para inicializar el estado y ejecutar cargas iniciales desde Firebase y SQLite. Las funciones declaradas en el contexto encapsulan la lógica de los servicios de base de datos (\texttt{ingredient-db-services.ts}, \texttt{recipe-db-services.ts}, \texttt{weeklyMeals-db-services.ts}, etc.), de modo que los componentes de la interfaz no necesitan conocer los detalles de la persistencia. Esta separación permite modificar el almacenamiento o la lógica de sincronización sin afectar a las pantallas. Se eligió Context API en lugar de Redux porque proporciona un mecanismo más ligero y suficiente para compartir estado entre pantallas en una aplicación del tamaño de NutriPlan. Además, facilita la creación de hooks personalizados y simplifica el testing al poder inyectar versiones simuladas durante las pruebas. La combinación de React Context, Firebase y SQLite garantiza consistencia entre el modo offline y el modo en línea, permitiendo que la aplicación funcione sin conexión y se sincronice cuando recupere la conectividad.

\chapter{Prototipos y desarrollo}\label{cap.prototipos y desarrollo}
La aplicación se desarrolló a partir de un único prototipo que evolucionó de forma iterativa. Cada etapa añadió nuevas capacidades y refinó las existentes, siguiendo la filosofía de mejora continua. A continuación se describen las principales fases de esa evolución:

	\section{Fase inicial: construcción del núcleo}
	Esta primera etapa se centró en validar la idea central de NutriPlan y en construir un núcleo funcional sobre el que crecer. Se implementó la autenticación de usuarios mediante Firebase Auth y se diseñó un esquema de datos básico en Firestore para almacenar recetas e ingredientes. Las primeras pantallas permitían crear y visualizar recetas, asociar ingredientes y consultar listados sencillos. La interfaz, minimalista y funcional, buscaba confirmar la viabilidad del modelo de datos y sentar las bases de la arquitectura en capas (presentación, servicios de datos y contexto global) que se mantendría en adelante. Esta fase, correspondiente a los primeros \textit{commits} (puntos de guardado) de \textit{Github} del proyecto, demostró que la propuesta era técnicamente factible y estableció la estructura de navegación y la gestión del estado global.
	
	\section{Fase de expansión: nuevas funcionalidades}
	Una vez asentado el núcleo, el desarrollo se centró en ampliar las funciones que convierten a NutriPlan en una herramienta completa de planificación de comidas. Se introdujo la \emph{gestión de la despensa} y de la \emph{lista de la compra} mediante nuevas tablas y servicios en la base de datos, así como pantallas dedicadas (\textit{PantryScreen} y \textit{GroceryListScreen}). Se añadió la \emph{planificación semanal}, permitiendo asignar recetas a cada día de la semana y generando automáticamente la lista de ingredientes necesaria. Para mejorar la búsqueda se integró un motor de indexado local basado en MiniSearch que ofrece autocompletado inmediato de ingredientes y recetas. Estos cambios implicaron reorganizar la navegación y refinar los estilos, y reflejan hitos intermedios del repositorio en los que se agregaron las tablas \texttt{ingredientPantry} y \texttt{weeklyMeals}, así como los servicios correspondientes. La aplicación pasó de ser un prototipo básico a un gestor completo de comidas gracias a estas incorporaciones.
	
	\section{Fase de refinamiento: optimización y mejoras avanzadas}
	La última fase de desarrollo se enfocó en pulir la experiencia y añadir características avanzadas a partir de la retroalimentación y el uso real. Se implementó la posibilidad de marcar ingredientes como comprados y sincronizar esos cambios con la despensa, creando el servicio \texttt{groceryBought} y automatizando la actualización del inventario. La autenticación se refinó mediante la introducción de la función \texttt{auth()}, que centraliza la inicialización de Firebase Auth y corrige problemas con la renovación de credenciales. Otra mejora significativa fue la incorporación de la subida y visualización de imágenes en las recetas utilizando \texttt{react-native-image-picker}, lo que aportó valor visual y una experiencia más rica. Estas mejoras fueron acompañadas de optimizaciones del código: uso intensivo de \texttt{useMemo}, \texttt{useCallback} y \texttt{useReducer} para reducir renderizados innecesarios, refactorización de servicios para unificar la lógica y la incorporación de pruebas automatizadas. Cada iteración consolidó el prototipo único, acercándolo a la versión final descrita en esta memoria, con un rendimiento optimizado y funcionalidades completas.
...
\chapter{Conclusions and future improvements}\label{cap.conclusions and future works}
	\section{Technical conclusions}
	From a technical perspective, \textit{NutriPlan} demonstrates the potential of React Native for rapid cross‑platform development. By leveraging Firebase as a serverless backend, the application benefits from scalable authentication, real‑time database capabilities and minimal infrastructure overhead. The architecture, based on separation of presentation components, data services and a global context layer, proved to be maintainable and easy to extend. The use of React hooks for state management avoided unnecessary complexity and aligned with a minimalist design philosophy. Performance was enhanced by memoization techniques and the adoption of \texttt{useMemo} and \texttt{useCallback} all of these technologies and designs provided confidence in the robustness and user experience of the app.
	
	\section{Personal conclusions}
	On a personal level, the project has been a profound learning experience. The author uses \textit{NutriPlan} every week, and it has become an invaluable tool to improve \textit{meal planning}, save time and reduce food waste. Throughout the development, new skills were acquired: integrating Firebase services, employing advanced React Native libraries, generating and installing APK files, testing applications on real devices and troubleshooting platform‑specific issues. The iterative refinement of a single prototype highlighted the importance of incremental improvement and continuous feedback, and it reinforced good practices such as clean code, documentation and careful interface design.
	
	\section{Future improvements}
	Looking ahead, several avenues for future work emerge. These include integrating external APIs for grocery pricing and nutrition data; implementing personalised recommendations using machine‑learning techniques; adding nutritional analysis and detailed statistics; supporting multiple languages and enhanced accessibility features; strengthening security with multi‑factor authentication and data encryption; and connecting with wearable devices and health services. Pursuing these directions will not only enrich the application but also offer opportunities for further research.
